...
<div class="container mx-auto mt-4 col-sm-12 col-md-12 col-lg-12 col-xl-12">
    <div class="wrapper rounded">
        <form method="get" action="{{ route('badge.alldelete') }}">
            @csrf
            <div class="table-responsive">
                <table id="example" class="table table-striped" style="width:100%">
                    ...
                    <tbody>
                        @if(count($badge)>0)
                        @foreach ($badge as $post)
                        <tr>
                            <td class="text-center">
                                <input type="checkbox" name="selectedItem[{{$post->id}}]" value="{{$post->id}}" id="">
                            </td>
                            <td>{{$post->id}}</td>
                            <td>{{$post->BadgeCode}}</td>
                            <td>{{$post->BadgeName}}</td>
                            <td>{{$post->updated_at}}</td>
                            <td>
                                <div class="sidebar">
                                    <a class="btn btn-primary menu my-2" href="{{route( 'badge.edit', $post->id)}}" role="button">Edit</a>
                                </div>
                                <a class="btn btn-danger" href="{{route( 'badge.delete', $post->id)}}" role="button">Delete</a>
